        ...
        if (!mCompletionOn) {
            if (mComposing.length() > 0) {
                switch (mComposing.toString().toLowerCase()){
                    case "russia": setSuggestions(new ArrayList<String>(Arrays.asList("你好")), true, true);
                    break;
                    default: setSuggestions(sugestões, true, true);
                }
                //ArrayList<String> list = new ArrayList<String>();
                //list.add(mComposing.toString());
                //setSuggestions(sugestões, true, true);
                //Log.i(TAG, "updateCandidates (depois de ter setsuggestions): se mcomposing.length > 0" + sugestões.toString());
                //setSuggestions(new ArrayList<String>(Arrays.asList("你好")), true, true);
            }
            else {
                setSuggestions(null, false, false);
